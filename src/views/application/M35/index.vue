<template>
  <div class="app-container">
    <h1>M35:申报教研课题、科研项目立项、验收和成果评价标准</h1>
    <br>
    <el-form ref="elForm" :model="formData" :rules="rules" size="medium" label-width="100px"
      ...
      <el-form-item label="三级分类" prop="thirdLevelType">
        <el-select v-model="formData.thirdLevelType" placeholder="请选择" clearable>
          <el-option
            v-for="(item,index) in thirdLevelOptions"
            :key="index"
            :label="item"
            :value="item"
          />
...
<script>
import { getFirstLevel, getSecondLevel,getThirdLevel, apply } from "@/api/application/M35";
...
export default {
  ...
  methods: {
    ...
    changeSelect1() {
      this.formData.thirdLevelType = ''
      getThirdLevel(this.formData.firstLevelType,this.formData.secondLevelType).then(response => {
        console.log(response)
        this.thirdLevelOptions = response
      })
    }
  },
}
