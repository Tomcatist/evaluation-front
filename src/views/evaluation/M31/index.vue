<template>
  <div class="app-container">
    ...
    <h1 style="align-content: center">M31:教学竞赛评价标准</h1>
    <br>
    ...
    <el-table v-loading="loading" :data="M31List" @selection-change="handleSelectionChange">
...
<script>
import { listM31, getM31, delM31, addM31, updateM31 } from "@/api/evaluation/M31";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listM31(this.queryParams).then(response => {
        this.M31List = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加教学竞赛评价标准";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getM31(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改教学竞赛评价标准";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateM31(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addM31(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除教学竞赛评价标准编号为"' + ids + '"的数据项？').then(function() {
        return delM31(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('evaluation/M31/export', {
        ...this.queryParams
      }, `M31_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
